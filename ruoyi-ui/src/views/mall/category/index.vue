...
<script>
import { listCategory, getCategory, delCategory, addCategory, updateCategory, treeListCategory } from "@/api/mall/category";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      treeListCategory(this.queryParams).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$modal.confirm('是否确认删除分类名称为"' + row.categoryName + '"的数据项？').then(function() {
